...
def get_liste_nuits():
    ...
    try:
        ...
        df = pd.read_sql(
            ...
            JOIN patient p 
            ON p.id_patient = n.id_patient

            JOIN resultat_nuit r 
            ON r.id_nuit = n.id_nuit
...
if not df_liste.empty:
    ...
    if search:
        ...
        filtered = filtered[
            filtered['nom'].str.contains(
                search,
                case=False,
                na=False
            )
            |
            filtered['prenom'].str.contains(
                search,
                case=False,
                na=False
            )
    ...
    if not filtered.empty:
        ...
        if not detail.empty:
            ...
            st.header(
                "Analyse IA : Comorbidités Probables"
            )
            ...
            if comorbidite:
                ...
                if st.button(
                    "Voir toutes les prédictions de comorbidités"
                ):

                    afficher_prediction_comorbidites(
                ...
                st.info(
                    "Pas assez de données pour la prédiction IA."
                )
            ...
            if rapport_path.exists():
                ...
                rapport = rapport_path.read_text(
                    encoding="utf-8"
                )
                ...
                st.warning(
                    f"Rapport non trouvé pour la nuit {selected_id}"
                )
        ...
        # =============================================================
            ...
            st.error(
                "Impossible de charger le détail de la nuit."
            )
